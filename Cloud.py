    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_channel):
            ...
            try:
                ...
                if stream_url:
                    if name not in youtube_results: youtube_results[name] = []
    ...
    for name in tv_order:
        if name in iptv_channels and iptv_channels[name]:
            ...
            if filename not in all_links:
                all_links[filename] = {'name': name, 'extinf': extinf_line, 'stream_url': url, 'type': 'iptv', 'filename': filename}
        if name in youtube_processed and youtube_processed[name]:
            ...
            if filename not in all_links:
                all_links[filename] = {'name': f"{name} (YouTube)", 'extinf': extinf_line, 'stream_url': stream_url, 'type': 'youtube', 'filename': filename}
    return all_links
...
def create_playlist(tv_order, iptv_channels, youtube_processed):
    ...
    for name, entries in iptv_channels.items():
        for entry in entries: pool.append({'name': name, 'data': entry, 'is_youtube': False})
    for name, entries in youtube_processed.items():
        for entry in entries: pool.append({'name': name, 'data': entry, 'is_youtube': True})
    playlist = ["#EXTM3U refresh=\"1\""]
    iptv_count = 0; youtube_count = 0; fallback_count = 0
    for channel_name in tv_order:
        ...
        for i, item in enumerate(pool):
            if item['name'] == channel_name:
                ...
                if item['is_youtube']: youtube_count += 1
                else: iptv_count += 1
                pool.pop(i); found = True; break
        if not found:
            playlist.extend([f'#EXTINF:-1 tvg-name="{channel_name}.tr" ,{channel_name}', FALLBACK_URL])
            fallback_count += 1
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as f: f.write("\n".join(playlist))
    return {'playlist_content': "\n".join(playlist), 'iptv_count': iptv_count, 'youtube_count': youtube_count, 'total_channels': (len(playlist) - 1) // 2, 'fallback_count': fallback_count}

# ==================== CLOUDFLARE VE PANEL ====================
def upload_to_cloudflare_with_links(playlist_content, stats, all_links):
    cf_config = config['cloudflare']
    url = f"https://api.cloudflare.com/client/v4/accounts/{cf_config['account_id']}/workers/scripts/{cf_config['worker_name']}"
...
<body>
    <div class="container">
        <header><h1><span class="logo">📺</span> IPTV API PANEL <span class="logo">⚡</span></h1><p style="color: #a0a0c0; font-size: 0.9rem; margin-top: 5px;">SEKOBES Bulut Yönetim Paneli</p></header>
        <div class="grid-layout">
            <div class="static-links-info"><p>📝 <strong>Not:</strong> {{STATIC_COUNT}} adet sabit link mevcut. <a href="/tv">/tv</a> ve <a href="/youtube">/youtube</a> sayfalarını ziyaret edin.</p></div>
            <div class="card blue"><div class="stat-header"><span style="font-size: 2.2rem;">📡</span><div><div class="stat-value">{{IPTV_COUNT}}</div><div class="stat-label">TV Kanalları</div></div></div></div>
            <div class="card pink"><div class="stat-header"><span style="font-size: 2.2rem;">🎥</span><div><div class="stat-value">{{YOUTUBE_COUNT}}</div><div class="stat-label">YouTube Kanalları</div></div></div></div>
            <div class="card indigo"><div class="url-title">🔗 Normal Playlist</div><div class="url-code" id="url1">{{WORKER_URL}}/playlist.m3u</div><div class="btn-group"><button class="btn btn-copy" onclick="copy('url1')">📋 Kopyala</button><a href="{{WORKER_URL}}/playlist.m3u" download="sekobes_playlist.m3u" class="btn btn-download">📥 İndir</a></div></div>
            <div class="card purple"><div class="url-title">🔄 Sabit Linkli Playlist</div><div class="url-code" id="urlStatic">{{WORKER_URL}}/sabit.m3u</div><div class="btn-group"><button class="btn btn-copy" onclick="copy('urlStatic')">📋 Kopyala</button><a href="{{WORKER_URL}}/sabit.m3u" download="sabit.m3u" class="btn btn-download">📥 İndir</a></div></div>
            <div class="card orange system-footer"><div><strong>DURUM</strong><span style="color: #4ade80;">● Aktif</span></div><div><strong>ALTYAPI</strong><span>Cloudflare Edge</span></div><div><strong>SAAT</strong><span id="currentTime">00:00:00</span></div></div>
        </div>
    </div>
    <script>
        function copy(id) { const text = document.getElementById(id).innerText; navigator.clipboard.writeText(text).then(() => { const el = document.getElementById(id); const original = el.innerText; el.innerText = "✅ Link Kopyalandı!"; setTimeout(() => { el.innerText = original; }, 1500); }); }
    ...
    html_content = html_template.replace('{{IPTV_COUNT}}', str(stats['iptv_count'])).replace('{{YOUTUBE_COUNT}}', str(stats['youtube_count'])).replace('{{STATIC_COUNT}}', str(len(all_links))).replace('{{WORKER_URL}}', cf_config['worker_url'])
...
const STATIC_PLAYLIST = (() => {{
  ...
  for (const [filename, d] of ALL_CHANNEL_LINKS) {{
    lines.push(d.extinf);
    lines.push(`{cf_config['worker_url']}${{d.type === 'iptv' ? '/tv/' : '/youtube/'}}${{filename}}`);
  }}
  return lines.join('\\n');
}})();
async function handleRequest(request) {{
  const url = new URL(request.url);
  if (url.pathname === '/sabit.m3u') return new Response(STATIC_PLAYLIST, {{ headers: {{ 'Content-Type': 'audio/x-mpegurl', 'Access-Control-Allow-Origin': '*' }} }});
  if (url.pathname === '/playlist.m3u') return new Response(PLAYLIST_CONTENT, {{ headers: {{ 'Content-Type': 'audio/x-mpegurl', 'Access-Control-Allow-Origin': '*' }} }});
  ...
  return new Response("Not Found", {{ status: 404 }});
    ...
    requests.put(url, headers=headers, data=worker_code.encode('utf-8'))
    return True
...
def upload_to_pythonanywhere():
    ...
    try:
        ...
        with open(OUTPUT_FILE, 'rb') as f:
            requests.post(pa_config['upload_url'], files={"file": ("SEKO5.m3u", f, "application/octet-stream")}, headers={"Authorization": f"Bearer {pa_config['token']}"}, timeout=10)
        return True
    except: return False
...
def main_process():
    ...
    iptv_channels = read_channels_file(IPTV_FILE); youtube_channels = read_channels_file(YOUTUBE_FILE, True); tv_order = read_order_file(TV_ORDER_FILE)
    ...
    stats = {'iptv_count': playlist_result['iptv_count'], 'youtube_count': playlist_result['youtube_count']}
    cloudflare_success = upload_to_cloudflare_with_links(playlist_result['playlist_content'], stats, all_links)
    ...
    print(f"✅ PythonAnywhere: {pa_durum}")
    print("=" * 50)
