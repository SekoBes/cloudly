...
@app.route("/debug")
def debug():
    """Sayfanin gercekte ne gosterdigini anlamak icin: sadece HTML metni (screenshot yok, bellek icin)."""
    dsmart_url = request.args.get("url")
    if not dsmart_url:
    ...
    def on_request(req):
        u = req.url
        if any(k in u for k in ["m3u8", "ercdn", "daioncdn", "erstream", "merlincdn", "token", "ticket"]):
    ...
    try:
        with sync_playwright() as p:
            ...
            result["body_text"] = page.inner_text("body")[:1000]
            result["relevant_requests"] = requests_seen[:20]
